...
SOLICITATION_PATTERNS = [
    r"\d+\s*명.*봐드",          # "5명 봐드릴게요"
    r"봐\s*드",                 # "봐드" 포함 전체 (봐드릴게요, 봐드려요, 봐드랴요 등 변형 모두)
    r"보\s*[ㅏ-ㅣ]\s*드",       # "보 ㅏ 드려요" (자음모음 분리 공격)
    r"풀어[드볼]",              # "풀어드릴게요"
    r"풀이",                    # "운세 풀이"
    r"댓글.*[주남달]",          # "댓글 주시면"
    r"댓\s*글",                 # "댓글", "댓 글" (단독으로도 유도 의도)
    r"오픈\s*채팅",
    r"카[카톡].*링크",
    r"DM\s*주",
    r"무료.*상담",
...
STANDALONE_SPAM_PATTERNS = [
    ...
    _NUM + r"\s*(명|분)\s*(만|정도|정두)?\s*보\s*[ㅏ-ㅣ]\s*드",  # 자음모음 분리
    r"(몇|몇몇)\s*(명|분)\s*(만)?\s*봐\s*드",
    r"(몇|몇몇)\s*(명|분)\s*(만)?\s*보\s*[ㅏ-ㅣ]\s*드",         # 자음모음 분리
    # "N명/분 봐볼/봐줄/보고" 류
    ...
    r"(가볍게|간단히|간단하게).*보\s*[ㅏ-ㅣ]\s*드",              # 자음모음 분리
    r"(가볍게|간단히|간단하게).*봐[볼줄]",
    r"(가볍게|간단히|간단하게).*보[고러]",
    # "신청" 류
    r"신청.*받.*" + _NUM + r"\s*(명|분)",
    ...
    # 기타
    r"도움.*필요.*봐",
    r"\d+\s*[-~]\s*\d+\s*명.*봐",           # "5-6명 봐드려요" (범위 + 봐)
    r"\d+\s*[-~]\s*\d+\s*(명|분).*보[고러]",  # "5-6명 보고가세요"
]
