...
STANDALONE_SPAM_PATTERNS = [
    ...
    r"취미로.{0,20}(제작|만들|봐|분석|풀이|해석|운|사주|타로|손금|관상|점)", 
    r"(해석|풀이|분석|상담|고민).{0,30}(open\.kakao\.com|카카오톡|오픈채팅|오픈카톡|chatgpt\.site)",
    r"(maeumgyeol|magickimm|chatgpt\.site)",
    
    r"(연애|결혼|이직|회사|직장|승진).{0,40}(운대|운때|나만의\s*때|누가\s*들어올지|언제\s*들어올지|인연은\s*있는지|운.{0,3}흐름|운이\s*들어|앞날을\s*[보봐파])",
    r"(각자만의|나만의|본인만의).{0,30}(운대|운때|흐름|때가).{0,20}(있는|궁금|없)"
]
