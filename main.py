...
# 한글 숫자 표현 대폭 확장 (네다섯, 두세 등 추가)
_NUM = r"(\d+|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|한두|두세|세네|네다섯|대여섯|서너)"

# 스패머가 쓰는 서술어 묶음 (봐, 보, 풀어, 해석, 확인, 파악, 풀이, 상담)
_VERB = r"(봐|보|풀어|해석|확인|파악|풀이|상담)"

# 단독 스팸 패턴 (코드가 훨씬 간결해지고 강력해집니다)
...
def check_spam_keyword(text: str) -> dict:
    ...
    for pattern in STANDALONE_SPAM_PATTERNS:
        if re.search(pattern, combined):
            return {
                "is_spam": True,
                "reason": "단독 스팸 패턴 감지 (모집, 취미 가장형, 악성 도메인 등)",
            }
