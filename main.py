...
# 확장된 포괄적 키워드 (띄어쓰기 없는 버전 및 은어 대폭 추가)
FORTUNE_KEYWORDS = [
    ...
    # 스패머가 쓰는 '운명/인연' 관련 은어들
    "운대", "운때", "누가 들어올지", "언제 들어올지", "나만의 때", "각자만의 흐름", "인연은 있는지",
    "운이 들어와", "운이 들어올지"
]

# 상담 유도 / 홍보 패턴 확장 (가짜 chatgpt 사이트 및 새로운 서술어 추가)
...
STANDALONE_SPAM_PATTERNS = [
    ...
    r"(해석|풀이|분석|상담|고민).*(open\.kakao\.com|카카오톡|오픈채팅|오픈카톡|chatgpt\.site)",
    
    # 문맥 상관없이 아래 악성 키워드나 도메인이 링크로 던져지면 즉시 스팸 처리
    r"(maeumgyeol|magickimm|chatgpt\.site)",
    
    # 신세 한탄을 가장한 밑밥(Bait) 투척형 방어 (이직, 회사, 승진 등 직장인 고민 포함)
    r"(연애|결혼|이직|회사|직장|승진).*(운대|운때|나만의\s*때|누가\s*들어올지|언제\s*들어올지|인연은\s*있는지|운.{0,3}흐름|운이\s*들어|앞날을\s*[보봐파])",
    r"(각자만의|나만의|본인만의).*(운대|운때|흐름|때가).*(있는|궁금|없)"
]
...
def check_spam_keyword(text: str) -> dict:
    ...
    for pattern in STANDALONE_SPAM_PATTERNS:
        if re.search(pattern, combined):
            return {
                "is_spam": True,
                "reason": "단독 스팸 패턴 감지 (취미/질문 가장형, 악성 도메인, 인원수 제한 등)",
            }
