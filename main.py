...
STANDALONE_SPAM_PATTERNS = [
    r"\d+\s*명\s*(만|정도|정두)?\s*봐드",      # "5명 봐드릴게요"
    r"(몇|몇몇)\s*(명|분)\s*(만)?\s*봐드",     # "몇 분만 봐드릴게여"
    r"(가볍게|간단히|간단하게).*봐드",           # "가볍게 봐드릴게요"
    r"도움.*필요.*봐드",                        # "도움 필요하신분? 봐드릴게여"
    r"\d+\s*명\s*(만|정도|정두)?\s*봐[볼줄]",   # "5명만 봐볼게요", "5명 봐줄게요"
    r"\d+\s*명\s*(만|정도|정두)?\s*보[고러]",   # "5명만 보고가세요"
    r"(몇|몇몇)\s*(명|분)\s*(만)?\s*봐[볼줄]",  # "몇 분만 봐볼게요"
    r"(몇|몇몇)\s*(명|분)\s*(만)?\s*보[고러]",  # "몇 분만 보고가세요"
    r"신청.*받.*몇\s*(명|분)",                  # "신청받을게요 몇 분만"
    r"몇\s*(명|분).*신청",                      # "몇 분 신청"
    r"\d+\s*명\s*(만|만요)?\s*$",               # "3명만요" (숫자+명으로 끝나는 짧은 글)
    r"신청.*받.*\d+\s*명",                      # "신청 받아요 3명"
    r"\d+\s*명.*신청",                          # "3명 신청받아요"
    r"(가볍게|간단히|간단하게).*봐[볼줄]",        # "간단히 봐볼게요"
    r"(가볍게|간단히|간단하게).*보[고러]",        # "간단히 보고가세요"
]
